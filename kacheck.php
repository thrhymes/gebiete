...
ORDER BY a1.`gebiet`";
  ...
  while ($row = mysqli_fetch_array($result)) {
    ...
    if     ( ( strtotime(date("Y-m-d")) - strtotime($rowIn) ) > 2592000 AND $show==1) {
...
$sqlString_1 = "SELECT a1.`gebiet`, a1.`out`, MAX(a1.`in`) AS `min`
FROM `aktionen` a1
WHERE a1.`gebiet` NOT IN (
    SELECT DISTINCT a2.`gebiet`
    FROM `aktionen` a2
...
ORDER BY a1.`gebiet`";
 ...
 $result_1 = mysqli_query($con,$sqlString_1);

  echo "<table width=90%>\n<caption>Gebiete die l&auml;nger als 6 Monate nicht bearbeitet wurden</caption>  <tr><th> # </th><th> Gebiet </th><th> Retour </th><th> Monate </th><th> im Umlauf </th></tr>\n";
